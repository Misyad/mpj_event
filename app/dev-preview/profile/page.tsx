...
export default function DevProfilePreviewPage() {
  ...
  const menuItems = [
    {
      href: '/dev-preview/profile/edit',
      title: 'Edit Profil',
      icon: PencilLine,
    },
    {
      href: '/dev-preview/profile/events',
      title: 'Riwayat Event',
      icon: CalendarDays,
    },
    {
      href: '/dev-preview/profile/certificates',
  ...
  return (
    <div className="flex flex-col pb-8 min-h-screen bg-[#f4f7f5] sm:max-w-md sm:mx-auto sm:border-x sm:border-black/5 sm:shadow-2xl relative">
      {/* Header */}
      <header className="sticky top-0 z-10 flex h-16 items-center justify-between gap-4 bg-[#f4f7f5]/90 px-4 backdrop-blur-md">
        ...
        {/* Soft Preview Badge */}
        <span className="inline-flex items-center rounded-full bg-amber-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-600 border border-amber-200">
          Preview
        </span>
      </header>
      ...
      <div className="px-4 py-4 space-y-6">
        ...
        <section className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="truncate text-base font-extrabold text-[#1B4332]">{mockUser.fullName}</h2>
              <p className="truncate text-xs font-medium text-gray-500 mt-0.5">{mockUser.whatsapp}</p>
              <p className="truncate text-xs font-medium text-gray-500">{mockUser.email}</p>
            </div>
          </div>
          <Link
            href="/dev-preview/profile/edit"
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link href="/dev-preview/profile/events" className="text-xs font-bold text-[#1B4332] hover:text-[#2d6a4f] transition">
              Lihat Semua
            </Link>
          </div>

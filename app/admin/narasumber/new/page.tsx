...
import { SpeakerCategory } from '@/types'
...
const defaultForm: FormData = {
  nama_lengkap: '', alamat: '', keahlian: [], keahlianInput: '',
  no_telp: '', portfolio_url: '', kategori: '', bio: '',
}
...
export default function NewSpeakerPage() {
  ...
  function removeKeahlian(k: string) {
    update('keahlian', form.keahlian.filter(x => x !== k))
  }
    ...
    <form onSubmit={handleSubmit} className="p-4 md:p-8 space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        <div className="space-y-1.5">
          ...
          <Input required value={form.nama_lengkap} onChange={e => update('nama_lengkap', e.target.value)}
            placeholder="Dr. Ahmad Faruqi, M.Kom" className="rounded-xl text-sm" />
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label className="text-xs font-semibold text-gray-600">Kategori <span className="text-red-400">*</span></Label>
            <Select value={form.kategori} onValueChange={v => v !== null && update('kategori', v as SpeakerCategory)}>
          ...
          <div className="space-y-1.5">
            ...
            <Input value={form.no_telp} onChange={e => update('no_telp', e.target.value)}
              placeholder="08111222333" className="rounded-xl text-sm" />
          </div>
        </div>

        <div className="space-y-1.5">
          <Label className="text-xs font-semibold text-gray-600">Alamat</Label>
          <Input value={form.alamat} onChange={e => update('alamat', e.target.value)}
            placeholder="Jakarta Selatan, DKI Jakarta" className="rounded-xl text-sm" />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Input value={form.portfolio_url} onChange={e => update('portfolio_url', e.target.value)}
            placeholder="https://dribbble.com/..." className="rounded-xl text-sm" />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Input
              ...
              onChange={e => update('keahlianInput', e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addKeahlian() }}}
              placeholder="Ketik keahlian lalu Enter atau klik +"
              className="rounded-xl text-sm flex-1"
            />
            <button type="button" onClick={addKeahlian}
          ...
          {form.keahlian.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {form.keahlian.map(k => (
                <span key={k} className="flex items-center gap-1 bg-gray-100 text-gray-700 text-xs px-2.5 py-1 rounded-full font-medium">
                  {k}
                  <button type="button" onClick={() => removeKeahlian(k)} className="text-gray-400 hover:text-red-500 transition-colors ml-0.5">

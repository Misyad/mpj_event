...
export function normalizeEvent(event: Partial<Event> & { id: string; title: string }): Event {
  return {
    ...
    payment_method: event.payment_method ?? event.paymentMethod ?? 'manual',
    paymentMethod: event.paymentMethod ?? event.payment_method ?? 'manual',
    gateway_provider: event.gateway_provider ?? event.gatewayProvider ?? null,
    gatewayProvider: event.gatewayProvider ?? event.gateway_provider ?? null,
    gateway_config: event.gateway_config ?? event.gatewayConfig ?? null,
    gatewayConfig: event.gatewayConfig ?? event.gateway_config ?? null,
